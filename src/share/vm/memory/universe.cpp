...
void Universe::genesis(TRAPS) {
  ...
  { FlagSetting fs(_bootstrapping, true);
    ...
    { MutexLocker mc(Compile_lock);
      ...
      // 计算虚函数表的大小 在这个表中，主是要一个类的虚函数的地址表，这张表解决了继承、覆盖的问题，保证其容真实反应实际的函数
...
void universe2_init() {
  ...
  // Although we'd like to verify here that the state of the heap is good, we can't because the main thread has not yet added itself to the threads list
  // (so, using current interfaces we can't "fill" its TLAB), unless TLABs are disabled.
  // 虽然我们想在这里验证堆的状态是否良好,但是不能。因为主线程还没有添加到线程集合
  // TODO
