...
/**所有klass类型共享的结构和行为：描述类型自身的布局,以及刻画出与其它类之间的关系*/
class Klass : public Klass_vtbl {
 ...
 protected:
  ...
  // where:
  //    tag is 0x80 if the elements are oops, 0xC0 if non-oops                若值为0x80表示数组元素类型是oop 值为0xC0则表示数组元素类型java基本类型
  ...
  //
  // Final note:  This comes first, immediately after Klass_vtbl,because it is frequently queried.
  // 反映该对象的整体布局的描述符 由于其类型是jint 针对32位x86其被压缩程32个比特位存储
  jint        _layout_helper;
  ...
  // 表示类型  实例类:java/lang/String等   数组类[I,[Ljava/lang/String;等
  Symbol*     _name;
 ...
 protected:
  ...
  // java/lang/Class instance mirroring this class 表示该Klass的Java层镜像类
  oop       _java_mirror;
  // Superclass 表示父类
  klassOop  _super;
  // First subclass (NULL if none); _subklass->next_sibling() is next one 指向第一个子类
  klassOop _subklass;
  // Sibling link (or NULL); links all subklasses of a klass  _subklass->_next_sibling可以找到下一个子类
 ...
 public:
  ...
  // Note: the prototype header is always set up to be at least the prototype markOop.
  // If biased locking is enabled it may further be biasable and have an epoch.
  //
