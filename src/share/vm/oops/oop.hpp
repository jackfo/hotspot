...
class oopDesc {
 ...
 private:
  volatile markOop  _mark;//对象头 涉及到线程状态、并发锁、GC分代信息
  union _metadata {
    wideKlassOop    _klass; //指向描述类型为Klass对象的宽指针
    narrowOop       _compressed_klass;//指向描述类型为Klass对象的压缩指针
  } _metadata;//元数据指针

  // Fast access to barrier set.  Must be initialized.
  static BarrierSet* _bs;//内存屏障
 ...
 public:
  ...
  // size of object header, aligned to platform wordSize 头部的个数
