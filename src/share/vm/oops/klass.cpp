...
klassOop Klass::base_create_klass_oop(KlassHandle& klass, int size,
  ...
  klassOop k  = kl->as_klassOop();//use klass to get klassOop
...
void* Klass_vtbl::operator new(size_t ignored, KlassHandle& klass,
                               int size, TRAPS) {
  // The vtable pointer is installed during the execution of constructors in the call to permanent_obj_allocate().
  // Delay the installation of the klass pointer into the new klass "k" until after the vtable pointer has been installed (i.e., until after the return of permanent_obj_allocate().
  // 在permanent_obj_allocate()调用的执行期间虚表指针会被安装
  // 延迟klass指针安装在创建一个新的klass在虚表指针安装之后,所以在这里调用permanent_obj_allocate_no_klass_install方法
  //
  klassOop k =
