...
// A Symbol is a canonicalized string. All Symbols reside in global SymbolTable and are reference counted.
// Symbol是规范化的字符串 所有的Symbols居与全局符号表是引用计数

// Reference counting
//
// All Symbols are allocated and added to the SymbolTable.
// When a class is unloaded, the reference counts of the Symbol pointers in the ConstantPool and in instanceKlass (see release_C_heap_structures) are decremented.
// When the reference count for a Symbol goes to 0, the garbage collector can free the Symbol and remove it from the SymbolTable.
// 所有的Symbols被分配和添加到SymbolTable
// 当一个类被卸载 在ConstantPool和instanceKlass中Symbol pointers的引用计数会减少 当Symbol的引用计数减为0垃圾回收器会释放并且移出它从SymbolTable
//
// 0) Symbols need to be reference counted when a pointer to the Symbol is saved in persistent storage.This does not include the pointer in the SymbolTable bucket (the _literal field in HashtableEntry)
//    that points to the Symbol.  All other stores of a Symbol* to a field of a persistent variable (e.g., the _name filed in FieldAccessInfo or _ptr in a CPSlot) is reference counted.
//    当一个Symbol的指针需要被保存持久存储 Symbols需要被引用计数
//
//
//
//
...
class Symbol : public CHeapObj {
 ...
 private:
  ...
  int            _identity_hash;
  unsigned short _length; // number of UTF8 characters in the symbol   symbol的长度
  jbyte _body[1];            //存放签名的具体内容
