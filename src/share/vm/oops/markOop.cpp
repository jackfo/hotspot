...
bool markOopDesc::should_not_be_cached() const {
  ...
  if (is_marked() && ((markOopDesc *)this)->decode_pointer() != NULL) {
    // If the oop containing this markOop is being forwarded, then we are in the middle of GC and we do not want the containing
    // oop to be added to a cache. We have no way of knowing whether the cache has already been visited by the current GC phase so
    // we don't know whether the forwarded oop will be properly processed in this phase. If the forwarded oop is not properly
    // processed, then we'll see strange crashes or asserts during the next GC run because the markOop will contain an unexpected value.
    //
    //
    //
    //
