...
public:
  unsigned int compute_hash(Symbol* name, Handle loader) {
    // Be careful with identity_hash(), it can safepoint and if this were one expression, the compiler could choose to unhandle each
    // oop before calling identity_hash() for either of them. If the first causes a GC, the next would fail
    // 获取符号的hash值和类加载器的hash值进行异或计算
    // .
