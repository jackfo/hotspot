...
_JNI_IMPORT_OR_EXPORT_ jint JNICALL JNI_CreateJavaVM(JavaVM **vm, void **penv, void *args) {
  ...
  // 同一时刻只能拥有一个java VM 因此一些运行状态是全局变量
  // We cannot use our mutex locks here, since they only work on
  // Threads. We do an atomic compare and exchange to ensure only
  // one thread can call this method at a time
  // 我们不能使用互斥锁在这里,因为他们工作线程。我们做一个原子比较和交换,确保一次只有一个线程可以调用这个方法
  // We use Atomic::xchg rather than Atomic::add/dec since on some platforms the add/dec implementations are dependent on whether we are running
  // on a multiprocessor, and at this stage of initialization the os::is_MP function used to determine this will always return false. Atomic::xchg
  // does not have this problem.
  // 我们使用Atomic::xchg而不是Atomic::add/dec 因为add/de实现依赖于我们是否正在运行在多处理器,
  // 在这个阶段的初始化os::is_MP函数用于确定这将总是返回false。Atomic::xchg就没有这个问题。
