...
VMOperationQueue::VMOperationQueue() {
  // The queue is a circular doubled-linked list, which always contains one element (i.e., one element means empty).
  // 队列是一个圆形doubled-linked列表,它总是包含一个元素一个元素意味着空的)。 在队列中创建相应的VM_Dummy
...
void VMThread::create() {
  ...
  _vm_thread = new VMThread();//创建一个新的虚拟机线程

  // Create VM operation queue
  _vm_queue = new VMOperationQueue();//创建线程操作队列
  guarantee(_vm_queue != NULL, "just checking");//检查_vm_queue 保证其不为空

  _terminate_lock = new Monitor(Mutex::safepoint, "VMThread::_terminate_lock", true);//设置监视器
