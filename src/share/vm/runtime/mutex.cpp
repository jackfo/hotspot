...
// Lock without safepoint check - a degenerate variant of lock().Should ONLY be used by safepoint code and other code
// that is guaranteed not to block while running inside the VM. If this is called with thread state set to be in VM, the safepoint synchronization code will deadlock!
//
//
