...
// Returns true if m must be compiled before executing it This is intended to force compiles for methods (usually for debugging) that would otherwise be interpreted for some reason.
// 如果方法在执行之前必须编译则返回真
//
bool CompilationPolicy::must_be_compiled(methodHandle m, int comp_level) {
  if (m->has_compiled_code()) return false;       // already compiled 检验方法是否被编译
