...
//------------------------------------------------------------------------------------------------------------------------
// In order to preserve oops during garbage collection, they should be allocated and passed around via Handles within the VM.
// A handle is simply an extra indirection allocated in a thread local handle area.
// 为了保证在垃圾回收期间对象的顺序,他们应当被分配和传递via句柄在VM
// 句柄是一个额外的间接分配一个线程本地处理区域。
// A handle is a ValueObj, so it can be passed around as a value, can be used as a parameter w/o using &-passing, and can be returned as a return value.
// 一个句柄是一个ValueObj,
//
//
...
//
//   oop obj = ...;
//   Handle h1(obj);              // allocate new handle 分配一个Handle
//   Handle h2(thread, obj);      // faster allocation when current thread is known 指定线程
