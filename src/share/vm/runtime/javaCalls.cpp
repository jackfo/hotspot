...
JavaCallWrapper::JavaCallWrapper(methodHandle callee_method, Handle receiver, JavaValue* result, TRAPS) {
  ...
  // Allocate handle block for Java code. This must be done before we change thread_state to _thread_in_Java_or_stub,since it can potentially block.
  //
...
void JavaCalls::call_helper(JavaValue* result, methodHandle* m, JavaCallArguments* args, TRAPS) {
  methodHandle method = *m;//获取方法句柄 对应调用参数
  JavaThread* thread = (JavaThread*)THREAD;//执行的java线程
  assert(thread->is_Java_thread(), "must be called by a java thread");//检测线程是否是java线程
  assert(method.not_null(), "must have a method to call");//检测方法是否为空
  assert(!SafepointSynchronize::is_at_safepoint(), "call to Java code during VM operation");//检测VM操作是否在安全点
  assert(!thread->handle_area()->no_handle_mark_active(), "cannot call out to Java here");//TODO
  ...
  // Ignore call if method is empty  检测如果方法为空 则直接返回 这里为空的意思 是方法里面内容
  ...
  assert(!thread->is_Compiler_thread(), "cannot compile from the compiler");//验证是否是编译线程,不能有编译线程执行java方法
  ...
  // Since the call stub sets up like the interpreter we call the from_interpreted_entry so we can go compiled via a i2c.
  //  Otherwise initial entry method will always run interpreted.
  //
