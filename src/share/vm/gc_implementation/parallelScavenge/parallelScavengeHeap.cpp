...
HeapWord* ParallelScavengeHeap::permanent_mem_allocate(size_t size) {
  assert(!SafepointSynchronize::is_at_safepoint(), "should not be at safepoint");//检查线程是否在安全点
  assert(Thread::current() != (Thread*)VMThread::vm_thread(), "should not be in vm thread");//检查侠士是否是VM线程
  assert(!Heap_lock->owned_by_self(), "this thread should not own the Heap_lock");//检查线程是Heap_lock是否是自己
  ...
  do {
    ...
    // total_collections() value!
    {
      MutexLocker ml(Heap_lock);//创建一个Heap_lock锁  m1
      gc_count      = Universe::heap()->total_collections();//获取从开始到现在gc次数
      full_gc_count = Universe::heap()->total_full_collections();//获取从开始到现在fullgc次数
