...
// ----------------------------------------------------------------------------
// Parallel class loading check 并行类加载器检查
...
klassOop SystemDictionary::resolve_or_null(Symbol* class_name, Handle class_loader, Handle protection_domain, TRAPS) {
  assert(!THREAD->is_Compiler_thread(), "Can not load classes with the Compiler thread");//检测线程类型 不能利用编译线程加载类
...
klassOop SystemDictionary::resolve_instance_class_or_null(Symbol* name, Handle class_loader, Handle protection_domain, TRAPS) {
  ...
  // UseNewReflection Fix for 4474172; see evaluation for more details
  // class_loader的类型是Handle 里面_handle是oop*
  class_loader = Handle(THREAD, java_lang_ClassLoader::non_reflection_class_loader(class_loader()));

  // Do lookup to see if class already exist and the protection domain has the right access
  //计算一个随机数并且获取随机数的索引 根据table_size进行计算
  unsigned int d_hash = dictionary()->compute_hash(name, class_loader);
  int d_index = dictionary()->hash_to_index(d_hash);
  klassOop probe = dictionary()->find(d_index, d_hash, name, class_loader,
                                      protection_domain, THREAD);//根据className查找对应的klassOop
  if (probe != NULL) return probe;


  // Non-bootstrap class loaders will call out to class loader and define via jvm/jni_DefineClass which will acquire the class loader object lock
  // to protect against multiple threads defining the class in parallel by accident.
  // This lock must be acquired here so the waiter will find  any successful result in the SystemDictionary and not attempt the define
  // ParallelCapable Classloaders and the bootstrap classloader,or all classloaders with UnsyncloadClass do not acquire lock here
  //
  // 非引导类加载器会调用类加载器并且jvm/jni_DefineClas需要加对象锁防止出现多线程定义类
  // lock在这里是必须的 所以会等待直到在SystemDictionary找到
  //
  //
  ...
  if (!class_has_been_loaded) {
    ...
    if (!class_has_been_loaded) {
      ...
      // clean up placeholder entries for success or error This cleans up LOAD_INSTANCE entries
      // It also cleans up LOAD_SUPER entries on errors from calling load_instance_class
      //
      //
      {
        MutexLocker mu(SystemDictionary_lock, THREAD);
...
instanceKlassHandle SystemDictionary::load_instance_class(Symbol* class_name, Handle class_loader, TRAPS) {
  instanceKlassHandle nh = instanceKlassHandle(); // null Handle 空句柄
  if (class_loader.is_null()) {
...
void SystemDictionary::initialize(TRAPS) {
  ...
  // Initialize basic classes 初始化基础的类
