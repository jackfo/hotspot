...
void ClassLoader::load_zip_library() {
  ...
  // First make sure native library is loaded 加载本地jar库
...
instanceKlassHandle ClassLoader::load_classfile(Symbol* h_name, TRAPS) {
  ...
  int classpath_index = 0;
  {
    ...
    ClassPathEntry* e = _first_entry;
    while (e != NULL) {
      stream = e->open_stream(name);//打开对应的class文件流
      if (stream != NULL) {
        break;
      }
      e = e->next();
...
// Initialize the class loader's access to methods in libzip.  Parse and process the boot classpath into a list ClassPathEntry objects.
...
int ClassLoader::compute_Object_vtable() {
  // hardwired for JDK1.2 -- would need to duplicate class file parsing code to determine actual value from file Would be value '11' if finals were in vtable
  //
  // 保留jdk1.2
