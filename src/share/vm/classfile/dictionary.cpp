...
// 这个程序不会锁定系统字典
// Since readers don't hold a lock, we must make sure that system dictionary entries are only removed at a safepoint (when only one thread is running),
// and are added to in a safe way (all links must be updated in an MT-safe manner).
// 因为读取不会有锁 我们必须保证system dictionary entries 在安全点移出(仅仅一个线程在运行)
// 必须通过安全的方式添加
//
// Callers should be aware that an entry could be added just after _buckets[index] is read here, so the caller will not see the new entry.
//
DictionaryEntry* Dictionary::get_entry(int index, unsigned int hash,
                                       Symbol* class_name,
                                       Handle class_loader) {
  oop loader = class_loader();//获取对应的类加载器
  debug_only(_lookup_count++);
  for (DictionaryEntry* entry = bucket(index);//获取游标对应的DictionaryEntry 如果hash值和class_name loader相同返回相应字典入口
                        entry != NULL;
                        entry = entry->next()) {
