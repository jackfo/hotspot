...
oop java_lang_ClassLoader::non_reflection_class_loader(oop loader) {
  if (loader != NULL) {
    // See whether this is one of the class loaders associated with the generated bytecodes for reflection,
    // and if so, "magically" delegate to its parent to prevent class loading from occurring in places where applications using reflection didn't expect it.
    // 如果loader为空 通过反射自动生成一个类加载器
    //
