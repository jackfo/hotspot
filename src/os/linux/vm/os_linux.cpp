...
bool os::create_thread(Thread* thread, ThreadType thr_type, size_t stack_size) {
  assert(thread->osthread() == NULL, "caller responsible");//验证线程不为空

  // Allocate the OSThread object
  OSThread* osthread = new OSThread(NULL, NULL);//创建对应的操作系统线程
  if (osthread == NULL) {
    return false;
  }

  // set the correct thread state 设置线程类型
  osthread->set_thread_type(thr_type);

  // Initial state is ALLOCATED but not INITIALIZED //设置线程状态
  osthread->set_state(ALLOCATED);

  thread->set_osthread(osthread);//设置系统线程对应的线程

  // init thread attributes 初始化线程属性
  pthread_attr_t attr;
